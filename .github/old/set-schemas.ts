...
const main = async () => {
  const apps = (await promises.readdir(`${__dirname}/../../apps`)).filter(
  ...
  for (const app of apps) {
    ...
    if (await existsAsync(appConfigPath)) {
      try {
        const appConfig = JSON.parse(
          await promises.readFile(appConfigPath, "utf8"),
        );
    ...
    if (await existsAsync(appDynamicComposePath)) {
      try {
        const appDynamicCompose = JSON.parse(
          await promises.readFile(appDynamicComposePath, "utf8"),
        );
        ...
        console.error(
          `Error setting dynamic compose schema for ${app}, error: ${e}`,
        );
      }
    }
  }
